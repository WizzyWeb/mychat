...
CHATWOOT_HUB_URL="https://hub.2.mychat.ae/events"
...
Join the community at https://mychat.ae/community?utm_source=cwctl
***************************************************************************
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.mychat.ae/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
https://www.mychat.ae/docs/deployment/deploy-chatwoot-in-linux-vm?utm_source=cwctl
...
Get help, https://mychat.ae/community?utm_source=cwctl
